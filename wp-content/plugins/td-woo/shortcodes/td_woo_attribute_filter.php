	...
	public function get_custom_css() {
		...
		$raw_css = "
			<style>
                ...
                body.woocommerce div.$unique_block_class .button-tdw-filter-item:last-child {
                    margin-right: 0;
                }
                /* @but_padd */
                body .$unique_block_class .button-tdw-filter-item {
	...
	function render($atts, $content = null) {
        ...
        if( $clear_all_filters_icon != '' ) {
            if( base64_encode( base64_decode( $clear_all_filters_icon ) ) == $clear_all_filters_icon ) {
                $clear_all_filters_icon_html = '<span class="tdw-filters-clear-all-icon tdw-filters-clear-all-icon-svg">' . base64_decode( $clear_all_filters_icon ) . '</span>';
            } else {
		...
		$buffy = '<div class="' . $this->get_block_classes() . '" ' . $this->get_block_html_atts() . $data_atts . '>';
			...
			$buffy .= '<div id=' . $this->block_uid . ' class="tdw-block-inner td-fix-index">';
		        ...
		        if ( array_filter( $selected_filters ) ) {
			        $selected_filters = array_combine( array_map( function($k) { return 'tdw_' . $k; }, array_keys( $selected_filters ) ), $selected_filters );
                }
