...
class tds_config {
			...
			array(
                ...
                'tdc_start_values' => base64_encode(
                    json_encode(
						array(
							array(
								'tds_plans_switcher1-annual_txt' => 'Yearly pricing',
								'tds_plans_switcher1-monthly_txt' => 'Monthly pricing',
							)
						)
                    )
                ),
		...
		td_api_block::add( 'tds_menu_login',
			array(
				...
				'params' => array_merge(
					...
					td_config_helper::get_map_block_shadow_array('menu_shadow', 'Shadow', 6, 0, 2, 'Style', '', 0, true, '', '', '', 'disable-dropdown' ),
					array(
						array(
							"param_name" => "separator",
							"type" => "text_separator",
					...
					td_config_helper::get_map_block_font_array( 'f_uh', true, 'Header text', 'Style', '', '', '', '', 'disable-dropdown' ),
					td_config_helper::get_map_block_font_array( 'f_links', false, 'Links text', 'Style', '', '', '', '', 'disable-dropdown' ),
					td_config_helper::get_map_block_font_array( 'f_uf', false, 'Footer text', 'Style', '', '', '', '', 'disable-dropdown' ),
					array(
						array(
							"param_name" => "separator",
							"type" => "text_separator",
					...
					td_config_helper::get_map_block_font_array( 'f_gh', true, 'Header text', 'Style', '', '', '', '', 'disable-dropdown' ),
					td_config_helper::get_map_block_font_array( 'f_btn1', false, 'Login button text', 'Style', '', '', '', '', 'disable-dropdown' ),
					td_config_helper::get_map_block_font_array( 'f_btn2', false, 'Signup button text', 'Style', '', '', '', '', 'disable-dropdown' ),
	...
	static function get_plans($type, $for_config = true) {
		if (is_null(self::$month_plans) || is_null(self::$year_plans) || is_null(self::$free_plans)) {
			self::$month_plans = [];
			self::$year_plans = [];
			self::$free_plans = [];
			...
			if ( null !== $plans) {
				...
				foreach ($plans as $plan) {
					...
					switch ($plan->months_in_cycle) {
						case '1': self::$month_plans[$plan->id] = [
							'name' => $plan->name,
							'price' => $plan->price,
							'months_in_cycle' => $plan->months_in_cycle,
							'trial_days' => $plan->trial_days,
							'is_free' => $plan->is_free,
						];
						break;

						case '12': self::$year_plans[$plan->id] = [
							'name' => $plan->name,
							'price' => $plan->price,
							'months_in_cycle' => $plan->months_in_cycle,
							'trial_days' => $plan->trial_days,
