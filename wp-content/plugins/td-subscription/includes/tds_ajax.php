					...
					*/
					...
					if ( $stripe_subscription ) {
						...
						if ( $stripe_subscription_status === 'trialing' ) {
							//$stripe_payment_status .= ' - ' . $stripe_subscription_status;
					...
					if ( $event->type === 'invoice.payment_succeeded' ) {
						...
						if ( in_array( $billing_reason, array( 'subscription_cycle', 'subscription_update' ) ) ) {
							$data['status'] = 'active';
							$data_format[] = '%s';
		...
		if ( empty( $result ) ) {
			...
			if ( !$valid_plan ) {
				...
				if ( empty( $result ) ) {
                    ...
                    if ( $list_id !== '' && $data_values['billing_email'] != '' ) {

                        $found_post = get_page_by_title( $data_values['billing_email'],'', 'tds_email' );
                        if ( $found_post === null ) {
                            $args = [
	...
	private static function get_all_plans(WP_REST_Request $request) {
		...
		if ( null !== $results) {
			foreach ( $results as &$item ) {
				...
				$item['publishing_limits_unserialized'] = unserialize($item['publishing_limits']);
			}
...
//		}
	...
	static function on_ajax_apply_coupon( WP_REST_Request $request ) {
		...
		// get coupon data
		$coupon = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM tds_coupons WHERE name LIKE %s LIMIT 1", $coupon_name ), ARRAY_A );
